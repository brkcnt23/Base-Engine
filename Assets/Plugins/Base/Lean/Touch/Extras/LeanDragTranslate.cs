...
namespace Lean.Touch
{
    ...
    public class LeanDragTranslate : MonoBehaviour
    {
        ...
        [ShowIf("@MovementClamp.x == 0")]
        [SerializeField] bool XClamp;
        [ShowIf("@MovementClamp.y == 0")]
        [SerializeField] bool YClamp;
        [ShowIf("@MovementClamp.z == 0")]
        [SerializeField] bool ZClamp;
        ...
        private void Translate(Vector2 screenDelta)
        {
            ...
            if (camera != null)
            {
                ...
                if (MovementClamp.x == 0) {
                    if (!XClamp) pos.x = transform.position.x;
                    else pos.x = Mathf.Clamp(camera.ScreenToWorldPoint(screenPoint).x, -MovementClamp.x, MovementClamp.x);
                }
                else {
                    pos.x = Mathf.Clamp(camera.ScreenToWorldPoint(screenPoint).x, -MovementClamp.x, MovementClamp.x);
                }
                if (MovementClamp.y == 0) {
                    if (!YClamp) pos.y = transform.position.y;
                    else pos.y = Mathf.Clamp(camera.ScreenToWorldPoint(screenPoint).y, -MovementClamp.y, MovementClamp.y);
                }
                else {
                    pos.y = Mathf.Clamp(camera.ScreenToWorldPoint(screenPoint).y, -MovementClamp.y, MovementClamp.y);
                }
                if (MovementClamp.z == 0) {
                    if (!ZClamp) pos.z = transform.position.z;
                    else pos.z = Mathf.Clamp(camera.ScreenToWorldPoint(screenPoint).z, -MovementClamp.z, MovementClamp.z);
                }
                else {
                    pos.z = Mathf.Clamp(camera.ScreenToWorldPoint(screenPoint).z, -MovementClamp.z, MovementClamp.z);
                }
